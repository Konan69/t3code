...
describe("IncusMachineService", () => {
  ...
  it.effect("writes a deterministic executable machine shim with mode 0755", () => {
    const spawner = ChildProcessSpawner.make((input) => {
      const command = input as unknown as { args: ReadonlyArray<string> };
      return Effect.succeed(
        makeHandle({
          stdout: command.args.includes("getent")
            ? "kixey:x:1000:1000::/home/kixey:/bin/bash\n"
            : "",
        }),
      );
    });
    ...
    return Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const machines = yield* MachineService;
      const firstPath = yield* machines.ensureExecutableShim({
        binding,
        command: "/host/bin/claude",
      });
      const firstContents = yield* fileSystem.readFileString(firstPath);
      const secondPath = yield* machines.ensureExecutableShim({
        binding,
        command: "/host/bin/claude",
      });
      const secondContents = yield* fileSystem.readFileString(secondPath);
      const info = yield* fileSystem.stat(secondPath);

      expect(firstPath).toBe(secondPath);
      expect(firstPath).toMatch(/\/machine-shims\/thread-thread-1\/claude$/u);
      expect(firstContents).toBe(
        IncusMachineService.buildMachineExecutableShim({
          machineName: binding.machineName,
          hostWorkspaceRoot: binding.hostWorkspaceRoot,
          guestWorkspaceRoot: binding.guestWorkspaceRoot,
          guestUid: "1000",
          guestGid: "1000",
          guestCommand: "claude",
          proxyPort: 3773,
    ...
    }).pipe(Effect.provide(Layer.merge(provideIncus(spawner), NodeServices.layer)));
  });

  it.effect("maps shim cwd and forwards the SDK environment with guest PATH and HOME", () => {
    const spawner = ChildProcessSpawner.make((input) => {
      const command = input as unknown as { args: ReadonlyArray<string> };
      return Effect.succeed(
        makeHandle({
    ...
    return Effect.gen(function* () {
      ...
      const tempRoot = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "t3-machine-shim-runtime-",
      });
      const hostWorkspaceRoot = path.join(tempRoot, "workspace");
      const hostSubdirectory = path.join(hostWorkspaceRoot, "packages", "server");
      const outsideDirectory = path.join(tempRoot, "outside");
      const fakeBin = path.join(tempRoot, "bin");
      yield* fileSystem.makeDirectory(hostSubdirectory, { recursive: true });
      yield* fileSystem.makeDirectory(outsideDirectory, { recursive: true });
      yield* fileSystem.makeDirectory(fakeBin, { recursive: true });
      const fakeIncus = path.join(fakeBin, "incus");
      yield* fileSystem.writeFileString(fakeIncus, "#!/usr/bin/env bash\nprintf '%s\\n' \"$@\"\n");
      yield* fileSystem.chmod(fakeIncus, 0o755);
      ...
      const shimPath = yield* machines.ensureExecutableShim({
        binding,
        command: "/host/bin/claude",
      });
      const environment = {
        ...
        T3_MCP_SERVER_URL: "http://localhost:3773/mcp",
      };
      const runShim = (cwd: string) => {
        const result = NodeChildProcess.spawnSync(shimPath, ["--print", "hello"], {
          cwd,
          env: environment,
          encoding: "utf8",
        });
      ...
      const argumentAfter = (args: ReadonlyArray<string>, flag: string) =>
        args[args.indexOf(flag) + 1];

      const rootArgs = runShim(hostWorkspaceRoot);
      expect(argumentAfter(rootArgs, "--cwd")).toBe("/home/kixey/ws");

      const subdirectoryArgs = runShim(hostSubdirectory);
      expect(argumentAfter(subdirectoryArgs, "--cwd")).toBe("/home/kixey/ws/packages/server");

      const outsideArgs = runShim(outsideDirectory);
      expect(argumentAfter(outsideArgs, "--cwd")).toBe("/home/kixey/ws");
      const forwardedEnvironment = new Map<string, string>();
      for (const [index, argument] of outsideArgs.entries()) {
        if (argument !== "--env") continue;
        const entry = outsideArgs[index + 1]!;
        forwardedEnvironment.set(entry.slice(0, entry.indexOf("=")), entry);
      }
      expect(forwardedEnvironment.get("KEEP_ME")).toBe("KEEP_ME=forwarded");
      expect(forwardedEnvironment.get("ANTHROPIC_BASE_URL")).toBe(
        "ANTHROPIC_BASE_URL=https://api.example.com",
      );
      expect(forwardedEnvironment.get("T3_MCP_SERVER_URL")).toBe(
        "T3_MCP_SERVER_URL=http://127.0.0.1:3773/mcp",
      );
      expect(forwardedEnvironment.get("PATH")).toBe(
        "PATH=/home/kixey/.local/bin:/home/kixey/.local/share/pnpm:/home/kixey/.bun/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",
      );
      expect(forwardedEnvironment.get("HOME")).toBe("HOME=/home/kixey");
      expect(outsideArgs.slice(outsideArgs.lastIndexOf("--"))).toEqual([
        "--",
        "claude",
        "--print",
        "hello",
